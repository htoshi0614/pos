# 配色修正 v2 を残りの派生画面にも適用
# 除外: stripe_service (支払い方法決定画面)、適用済み: point_mail/closing/data_import
...
def main():
    files = ["backup_service.py", "bottle_keep.py", "cast_salary.py",
             "customer_crm.py", "management.py", "pricing_engine.py",
             "tab_management.py", "weather_service.py"]
    roots = [
    ...
    for root in roots:
        for f in files:
            p = root / f
            if p.exists():
                log.append(("[patched] " if patch(p) else "[skip] ") + f + " @ " + root.name)
    (Path(r"C:\Users\htosh\OneDrive\デスクトップ\posstart") / "_v3log.txt").write_text(chr(10).join(log), encoding="utf-8")
